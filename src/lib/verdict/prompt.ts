...
8. unclear면 **무엇을 확인하면 판정이 갈리는지**를 checks에 짧은 명사구로 적는다.
   원문이 요구하는데 사용자 조건으로는 확인할 수 없는 항목만 넣는다. 이미 충족을 확인한 조건은 넣지 않는다.
   원문에 자격 조건 자체가 적혀 있지 않아 확인할 항목조차 없으면 checks를 빈 배열로 둔다.
   eligible·ineligible이면 checks는 빈 배열이다.
