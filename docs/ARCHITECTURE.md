...
게이트를 13,662건에 붙여 측정한 결과다 (대표 프로필 28세·서울·근로자·1인가구, 목록 화면 888건 기준).

| 그룹 | 단독 탈락 | 내역 | 판단 |
|---|---|---|---|
| 개인상황 | 99건 | **구직자/실업자 68** · 장애인 24 · 대학생 5 · 기타 5 | 69%가 정당한 배제 → **유지** |
...
1. 정당한 배제가 69%다 — 근로자에게 `미취업청년 지원사업`은 진짜로 해당 없다
2. 남은 31건(장애인·보훈·질병)은 **사용자가 스스로 체크할 동기가 강한** 항목이다. 폼 안내로 회수된다
3. 이걸까지 빼면 888건 화면에서 게이트가 확정하는 게 사실상 없어진다 — 나이·지역은 SQL 1차 필터와 중복이므로 2단 게이트가 무의미해진다 (PRD §7.2의 "AI 호출이 줄어든다"가 0)

> **성격의 차이가 아니라 정도와 노출의 차이다.** 잔여 오판 위험 3.5%(31/888)를 감수한 트레이드오프이고,
> 이걸 원칙으로 위장하지 않는다. 회수 장치는 셋이다 — 프롬프트 규칙 6번(§5.1), 폼 안내(§6.3), 블로커 문구(§7.5).

게이트 통과가 729 → **784건**으로 늘었다. `무주택세대` 정책 43건이 `아님` 대신 AI 판정 대상이 된다. 판정은 페이지 10건 단위이므로(PRD §7.7) **클릭당 호출 증가분은 1건 미만이다.**

> 이후 §5.0.3의 사용자구분 검사가 추가되어 같은 화면의 게이트 통과는 **514건**이 됐다.
...
| 항목 | 값 |
|---|---|
| 모델 | `gemini-2.5-flash` — **구현 시점에 사용 가능한 모델 목록을 확인해 확정** |
