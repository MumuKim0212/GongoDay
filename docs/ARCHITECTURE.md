...
│ [검색____] [출처 ▾]  전체 / 스크랩          │
│ [이 페이지 10건 판정하기]  해당 3·애매 5·아님 2│  ← 판정 상태를 들고 있는 클라이언트 경계
├────────────────────────────────────────────┤
│ ✅해당 [청년] 청년월세 특별지원              │
...
- **판정은 현재 페이지 10건만.** 페이지를 넘기면 다시 눌러야 한다. 다만 저장된 판정은 목록 조회 시 `verdicts`를 함께 읽으므로 **한 번 판정한 페이지는 다시 눌러도 Gemini 호출이 0건이다**
- **판정 버튼은 목록 바로 위에 둔다.** 버튼과 카드가 판정 상태를 공유해야 해서 한 클라이언트 컴포넌트(`PolicyList`) 안에 있고, 버튼이 미는 대상이 바로 아래 10건이라 자리도 그쪽이 맞다. 상단에는 프로필·갱신만 남는다
