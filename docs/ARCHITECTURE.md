...
| 가구상황 | `codes.household`가 비지 않았고 `JA0410` 미포함이며 프로필 가구와 교집합 없음 |
| 사업자 | `codes.business`가 비지 않았고 프로필 사업자상태가 없음 |

**`no_limit`에 그룹명이 있으면 그 그룹은 무조건 통과**한다 (§2.1.1). `JA0322`/`JA0410`(해당사항없음)도 제한 없음으로 읽는다. 의미를 아는 코드만 이렇게 쓰고, `unknown`은 읽지 않는다.
...
- **나이·지역·분야만 SQL에 넣는다.** 나머지(성별·소득·상황·가구·사업자)는 배열 교집합이라 SQL이 복잡해지고 **게이트/SQL 중복 지점이 늘어난다.** 중복은 최소로 유지한다
