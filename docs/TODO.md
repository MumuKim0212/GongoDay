...
- [x] **가구상황을 게이트에서 제외** ([§5.0.2](ARCHITECTURE.md)) — 단독 탈락 55건 중 43건이 `무주택세대`였다
- [x] 시스템 프롬프트 규칙 7번 추가 — 다중선택 항목을 완전하다고 가정하지 않는다 ([§5.1](ARCHITECTURE.md))

> ⚠️ **게이트가 실데이터에서 두 소스에 같은 비율로 걸렸다** (gov24 16.6% / youth 16.9%). 소스 분기 없이
> 같은 함수가 양쪽을 처리한다는 설계([§2.1.1](ARCHITECTURE.md))가 성립했다.

> **남은 오판 위험 3.5%** — 개인상황의 부가 코드(장애인 26·보훈 3·질병 1 등). 회수 장치 셋 중 둘이 아직 없다:
> **폼 안내(작업 4)** 와 **라벨 붙은 블로커 문구(작업 6)**. 둘 다 하기 전에는 이 위험이 남아 있다.
